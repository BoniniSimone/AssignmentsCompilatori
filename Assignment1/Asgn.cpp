...
namespace {
    ...
    PreservedAnalyses run(Function &F, FunctionAnalysisManager &) {
      ...
      for (auto &BB : F) {
        for (auto Inst = BB.begin(); Inst != BB.end(); ) {
          ...
          if (auto *BinOp = dyn_cast<BinaryOperator>(I)) {  // Considera solo operazioni binarie (Add, Mul, ecc.)
            
            // Caso: addizione (x + 0 oppure 0 + x)
            if (BinOp->getOpcode() == Instruction::Add) {
              Value *Op;  // Variabile per contenere l'operando non nullo
              if (match(BinOp, m_Add(m_Value(Op), m_Zero()))) {  //m_add controlla se il primo operando è un valore e il secondo è zero. Mentre match è una funzione che verifica se l'istruzione corrisponde al pattern specificato.
  ...
  // Riconosce: 16 * x e lo sostituisce con (x << 4)
  // Riconosce: x / 16 e lo sostituisce con (x >> 4)
  //-----------------------------------------------------------------------------
        ...
        for (auto Inst = BB.begin(); Inst != BB.end(); ) {
          ...
          if (auto *MulInst = dyn_cast<MulOperator>(I)) { 
            ...
            if (ConstOperand && VarOperand) {
              int ConstValue = ConstOperand->getSExtValue();  //getSExtValue() restituisce il valore del costante intero
